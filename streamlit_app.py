...
.stButton button { border-radius: 12px; background: white; border: 1px solid #e2e8f0; color: #1e3a5f; padding: 8px 12px; font-size: 14px; }
.stButton button:hover { background: #1e3a5f; color: white; }
.form-container { background: white; padding: 20px; border-radius: 25px; margin-top: 20px; }
...
/* ESTILO PARA MÓVIL - NÚMEROS EN CUADRÍCULA ORDENADA */
@media (max-width: 768px) {
    .stButton button { font-size: 12px; padding: 6px 8px; min-width: 45px; }
    .premio-card { padding: 10px 5px; font-size: 12px; }
    .main-title { font-size: 1.8em; }
}

/* CONTENEDOR DE NÚMEROS EN GRID */
.numeros-grid {
    display: grid;
...
# FUNCIÓN MEJORADA PARA MÓVIL - NÚMEROS ORDENADOS
st.markdown('<div class="numeros-grid">', unsafe_allow_html=True)

# Crear una fila de 10 columnas usando st.columns (funciona mejor en móvil)
filas = 10
...
if 'numero_seleccionado' in st.session_state:
    ...
    if estado_actual != "Disponible":
        ...
        with st.form("compra_form"):
            ...
            comprobante = st.file_uploader("Comprobante (opcional)", type=['png', 'jpg', 'pdf'])
